...
macOS Tauriアプリのウインドウタブは [Tauri macOS ウインドウタブ実装ガイド](./tauri-macos-tabs.md)
を正本とする。traffic lightと同じ38px行へ統合し、タブ切替ではネイティブWindowを
交換せず、同一Window内のchild WebViewだけを切り替える。
